...
# shellcheck disable=SC2034  # Variable is referenced indirectly
PYTHON_BANDIT_FILE_NAME="${PYTHON_BANDIT_CONFIG_FILE:-.bandit}"
# shellcheck disable=SC2034  # Variable is referenced indirectly
...
LANGUAGE_ARRAY=('ANSIBLE' 'ARM' 'BASH' 'BASH_EXEC' 'CLOUDFORMATION' 'CLOJURE' 'COFFEESCRIPT' 'CSHARP' 'CSS'
  ...
  'TEKTON' 'TERRAFORM' 'TERRAFORM_TERRASCAN' 'TERRAFORM_TFSEC' 'TERRAGRUNT' 'TSX' 'TYPESCRIPT_ES' 'TYPESCRIPT_STANDARD' 'XML' 'YAML')
...
LINTER_NAMES_ARRAY['TERRAFORM_TFSEC']="tfsec"
...
LINTER_COMMANDS_ARRAY['TERRAFORM_TFSEC']="tfsec --config-file ${TERRAFORM_LINTER_RULES}/config.yml --custom-check-dir=${TERRAFORM_LINTER_RULES} ${LINTER_OPTS[TERRAFORM_TFSEC]} -o "
